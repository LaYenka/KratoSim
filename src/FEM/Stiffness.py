...
class Stiffness():
	...
	# conn[0] is node numbers of the element
		...
		#               [0.0,   0.066]]
			...
			if len(nodePts) == 2:
				B = np.zeros((3,4))     # 
				Ke = np.zeros((4,4))    # element stiffness matrix is 8x8
				...
				for q in self.q2:		# for each Gauss point
					...
					J  = np.dot(dN, nodePts).T     # Jacobian - J is 2x2
					print(J,dN,nodePts,n_el)
					dN = 1/np.linalg.norm(J)*dN    # partial derivative of N wrt (x,y): 2x4
					print(dN)
					# assemble B matrix  [3x4]
					B[0,0] = dN[0]
					B[0,1] = dN[1]
					B[1,0] = dN[0]
					B[1,1] = dN[1]
					# element stiffness matrix (8X8)
					Ke += np.dot(np.dot(B.T,self.C_beam),B) * 1/np.linalg.norm(J)
