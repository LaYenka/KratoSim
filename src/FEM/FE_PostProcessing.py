...
class FE_PostProcessing:
	...
	# (pre-allocate space for nodal stress and strain)
		...
		for n_el in self.mesh_dict['Elem_num']:     # loop through each element
			...
			if len(nodePts) == 2:
					B = np.zeros((3,4))     # 
					print('linear element to be implemented for post-processing')
		...
		for ni,pt in enumerate(self.nodes):
			...
			if self.plot_type=='s11': res.append(node_stress[ni][0])		# s11
			if self.plot_type=='s22': res.append(node_stress[ni][1])		# s22
			if self.plot_type=='s12': res.append(node_stress[ni][2])		# s12
			if self.plot_type=='e11': res.append(node_strain[ni][0])		# e11
			if self.plot_type=='e22': res.append(node_strain[ni][1])		# e22
			if self.plot_type=='e12': res.append(node_strain[ni][2])		# e12
		tri = []
		for c in self.conn:
			if len(c) == 4:
